...
pub fn main() void
{
    ...
    {
        const zig = "/bin/zig";
        const execveCompileFlags: [*:null]const ?[*:0]const u8 = &.{zig, "build-exe", "src/ArchiveConverter.zig","--global-cache-dir","/tmp/.zig-cache-debug","--cache-dir","/tmp/.ArchiveConverter","-L","/usr/lib","-search_dylibs_only","-fno-llvm","-fno-ubsan-rt","-fstrip","-lc","-llz4",null};
        _ = linux.execve(zig, execveCompileFlags, @ptrCast(std.os.environ));
    }
    var status: u32 = undefined;
    _ = linux.waitpid(-1, &status, 0);
