...
pub fn main() void
{
    ...
    {
        const zig = "/bin/zig";
        const execveCompileFlags: [*:null]const ?[*:0]const u8 = &.{zig, "build-exe", "src/main.zig", "--name", TARGET,"--global-cache-dir","/tmp/.zig-cache-linux","--cache-dir","/tmp/.Age of Wonders 4","-target","x86_64-linux-gnu","-mcpu=x86_64_v3","-I", "/usr/include","-L","/usr/lib","-search_dylibs_only","-OReleaseSmall","-fstrip","-femit-asm","-lc","-lSDL3",null};
        _ = linux.execve(zig, execveCompileFlags, @ptrCast(std.os.environ));
    }
    var status: u32 = undefined;
    _ = linux.waitpid(-1, &status, 0);
